...
import asyncio     # For non-blocking execution of shell commands
...
DEFAULT_WORKSPACE = os.getenv(
    "WORKSPACE_DIR",
    "/Users/himanshu/github_himanshu/Model-Context-Protocol-MCP-/SSE_MCP/workspace"
)
...
# --------------------------------------------------------------------------------------
# TOOL 1: run_command — execute a shell command and return output (NON-BLOCKING)
# --------------------------------------------------------------------------------------
@mcp.tool()
async def run_command(command: str) -> str:
    ...
    # Internal blocking function that actually runs the command
    def _run() -> str:
        result = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=15  # Prevent long-running or stuck commands
        )
        return result.stdout or result.stderr or ""

    try:
        # Run the blocking command in a background thread so the event loop is not blocked
